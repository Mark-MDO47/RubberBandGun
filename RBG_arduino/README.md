...
* **FactorySettings:** the EEPROM stores four configurable settings for all the sounds and LED effects. In PROGMEM storage is the factory configuration for these four configurable settings. This tab organizes the factory settings and helps me generate the factory_effect_configs table. The original plan was to have four EEPROM configurations: a running configuration and three saved auxilliary configurations.
  * However it was more fun to cycle through the configuration automatically, so now there are four configurations in positions 0 thru 3 that cycle each time a rubber band is shot. Position 0 is the position used for editing.
