...
* **FactorySettings:** the PROGMEM stores four configurable settings for all the sounds and LED effects. If the checksum in EEPROM storage doesn't match the data, these four PROGMEM settings are copied into EEPROM; This is equivalent to a complete "Factor Reset" from the audio menus.
  * We cycle through the configurations automatically each time a rubber band is shot.
  ...
  * Factory Reset from the audio menus will set one or all four of these configurations back to the EEPROM default.
